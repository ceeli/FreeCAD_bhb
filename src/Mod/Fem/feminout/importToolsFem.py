...
def get_FemMeshObjectOrder(
    ...
    if edges != ():
        ...
        Console.PrintError("Found no edges in mesh: Element order determination does not work without them.")
...
def make_femmesh(
    ...
    if ('Nodes' in m) and (len(m['Nodes']) > 0):
        ...
        if (
            ...
            nds = m['Nodes']
            Console.PrintLog("Found: elements\n")
            for i in nds:
                n = nds[i]
