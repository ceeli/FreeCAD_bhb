...
class inp_writer:
    ...
    def write_element_sets_material_and_femelement_type(self, f):
        ...
        for mi, m in enumerate(self.material_objects):
            mat_obj = m['Object']
            if len(self.beamsection_objects) > 0:  # all fem_elements are beamsection_obj --> beam mesh
                ...
                for bi, b in enumerate(self.beamsection_objects):
                    ...
                    if len(beamsection_obj.References) == 0:
                ...
                f.write('\n')
            elif len(self.shellthickness_objects) > 0:  # all fem_elements are shells --> shell mesh
                ...
                for si, s in enumerate(self.shellthickness_objects):
                    ...
                    material_elementset_line = '*ELSET,ELSET=' + material_elementset_name + '\n'
                    if len(shellthickness_obj.References) == 0:
    ...
    def write_femelementsets(self, f):
        ...
        for m in self.material_objects:
            mat_obj = m['Object']
            mat_name = mat_obj.Material['Name'][:80]
            if len(self.beamsection_objects) > 0:   # all fem_elements are beams
                for b in self.beamsection_objects:
                    ...
                    f.write(el_prop)
                    f.write(sc_prop)
            elif len(self.shellthickness_objects) > 0:   # all fem_elements are shells
    ...
    def write_constraints_fixed(self, f):
        ...
        for fixed_object in self.fixed_objects:
            ...
            if len(self.beamsection_objects) != 0 or len(self.shellthickness_objects) != 0:
